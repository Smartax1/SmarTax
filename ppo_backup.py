...
class ppo_agent:
    def __init__(self, envs, args):
        ...
        self.net = mlp_net(self.envs.government.action_space.shape[0])
    ...
    def obs_concate_numpy(self, global_obs, update=True):
        return np.tile(global_obs, (self.args.n_households, 1))
    ...
    def learn(self):
        ...
        for update in range(self.args.n_epochs):
            ...
            for step in range(self.args.epoch_length):
                with torch.no_grad():
                    house_values, house_pis = self.net(self._get_tensor_inputs(self.obs))
                ...
                action = {self.envs.government.name: self.gov_action_max * (gov_actions * 2 - 1),
                          self.envs.households.name: np.full((10,2), 0)}
                ...
                for n, done in enumerate(dones):
                    if done:
                        ...
                        next_global_obs = self.observation_wrapper(next_global_obs)
                self.obs = self.obs_concate_numpy(next_global_obs)
        ...
        for epoch_i in range(self.args.eval_episodes):
            ...
            while True:
                with torch.no_grad():
                    ...
                    next_global_obs, _, gov_reward, _ = self.eval_env.step(action)
                    next_global_obs = self.observation_wrapper(next_global_obs)
    ...
    def _evaluate_get_action(self, global_obs):
        self.obs = self.obs_concate_numpy(global_obs)
        house_values, house_pis = self.net(self._get_tensor_inputs(self.obs))
        actions = select_actions(house_pis)
        gov_actions = np.random.random(self.envs.government.action_space.shape[0])
        action = {self.envs.government.name: self.gov_action_max * (gov_actions * 2 - 1)}
